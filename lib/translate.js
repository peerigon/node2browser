...
function loadFiles(filesToLoad, pathModifier, finalCallback) {
    var loadedFiles = {},
        pending = 0,
    ...
    function loadFile(path, callback) {
        function finished(err) {
            ...
            if (err) {
                console.log("Loaded files before error: ");
                console.log(Object.keys(loadedFiles));
                callback(err);
            } else {
                callback(null);
            }
        ...
        if (!pathModifier(path)) {  // IF TRUE: This file should be ignored
            callback(null);
            return;
        }
...
function translate(paths, pathModifier, callback) {
    function onFilesLoaded(err, loadedFiles) {
        ...
        if (err) {
            ...
            result = assembleStrings(loadedFiles);
    ...
    loadFiles(paths, pathModifier, onFilesLoaded);
}
