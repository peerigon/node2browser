...
function resolve(currentModuleDir, path, callback) {
    ...
    function loop() {
        if (i < lookUpPaths.length) {
            ...
            callback(new Error('Error in ' + currentModuleDir + ': Cannot find module ' + path));
        }
        i++;
    }
    ...
    if (/\.node$/i.test(path)) {
        callback(new Error('Error in ' + currentModuleDir + ': .node-files are not supported in the browser context.'));
    } else {
