...
function resolve(currentModuleDir, path, callback) {
    ...
    function loop() {
        if (i < lookUpPaths.length) {
            ...
            callback(new Error('Cannot find module ' + path + ' required in ' + currentModuleDir));
        }
        i++;
    }
    ...
    if (/\.node$/i.test(path)) {
        callback(new Error('In ' + currentModuleDir + ': ".node"-files are not supported in the browser context'));
    } else {
